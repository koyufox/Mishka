...
| KSP                           | 2.3.6         | 注解处理（Room）            |
| Compose Multiplatform         | 1.11.0        | 跨平台 UI 框架              |
| miuix                         | 0.9.1         | UI 组件库 + 导航            |
| miuix-blur                    | 0.9.1         | 模糊/着色器效果             |
| androidx.navigation3          | 1.1.1         | 类型安全路由                |
| Room                          | 3.0.0-alpha04 | 跨平台数据库（KMP）         |
| Ktor                          | 3.4.3         | HTTP/WebSocket 客户端       |
| kotlinx-coroutines            | 1.11.0        | 异步/并发                   |
| kotlinx-collections-immutable | 0.4.0         | UI state 持久不可变集合     |
...
| mihomo                        | v1.19.25+     | 代理核心（Mishka patch）    |
...
**ROOT 动态通知仅亮屏启用**：[DynamicNotificationManager.startOrFallbackStatic](android/src/main/kotlin/top/yukonga/mishka/service/DynamicNotificationManager.kt) 默认仍只允许 VPN 动态；ROOT 模式必须由 [MishkaRootService](android/src/main/kotlin/top/yukonga/mishka/service/MishkaRootService.kt) 传入 `allowRootDynamic=true` 才能走动态分支。`MishkaRootService` 动态注册 `ACTION_SCREEN_ON/OFF` receiver，以 `PowerManager.isInteractive` 初始化亮屏状态；亮屏且用户开启 `DYNAMIC_NOTIFICATION` 时显示动态流量通知，息屏时先 `dynamicNotification.stop()` 取消 `/traffic` 收集，再用同一通知 ID 切回静态通知。不要用 manifest 静态 receiver，也不要引入 wake lock。原因：VPN 模式靠 `BIND_VPN_SERVICE` 隐式让进程进入 `BOUND_FOREGROUND_SERVICE` 状态自动保 CPU、不被 battery optimization 限制；ROOT 模式无任何系统 binding，息屏后 device idle 会让 1Hz `/traffic` WS 帧合并、`NotificationManager.notify()` 批处理，动态通知冻结。`PARTIAL_WAKE_LOCK` 实测被系统 DISABLED 救不了（它只阻 SoC suspend 不阻 CPU idle）；亮屏限定是在承认平台限制的前提下保留可见时的实时体验。
